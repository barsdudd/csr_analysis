...
TString getQuery(TString run, TString run_R007){
  ...
  //  q1 += "AND kDimuon.mass BETWEEN 4.2 AND 8.7 ";
  //  q1 += "AND kDimuon.xT BETWEEN 0.05 AND 0.58 ";
  // For MC productions > R006, the beam offset has to be taken into consideration, so the cut changes to abs(dy - 1.6)<0.24                                                                                       
  // q1 += "AND ABS(kDimuon.dx) < 0.3 ";
  // q1 += "AND ABS(kDimuon.dy - 1.6) < 0.22 ";
  // //q1 += "AND ABS(kDimuon.dy - 0.4) < 0.22 ";
  // q1 += "AND ABS(kDimuon.dpx) < 2. ";
  // q1 += "AND ABS(kDimuon.dpy) < 2. ";
  // //q1 += "AND kDimuon.dx*kDimuon.dx + (kDimuon.dy - 0.4)*(kDimuon.dy - 0.4) < 0.09 "; RS 57, 59
  ...
  // //q1 += "AND MU_PLUS.xD*MU_PLUS.xD + (MU_PLUS.yD - 0.4)*(MU_PLUS.yD - 0.4)> 12. ";
  // //q1 += "AND MU_MINUS.xD*MU_MINUS.xD +(MU_MINUS.yD - 0.4)*(MU_MINUS.yD - 0.4) > 12. ";
  // //q1 += "AND MU_PLUS.xD*MU_PLUS.xD + (MU_PLUS.yD - 0.4)*(MU_PLUS.yD - 0.4)< 1200. ";
  // //q1 += "AND MU_MINUS.xD*MU_MINUS.xD +(MU_MINUS.yD - 0.4)*(MU_MINUS.yD - 0.4) < 1200. ";
  // //q1 += "AND MU_PLUS.xT*MU_PLUS.xT + (MU_PLUS.yT - 0.4)*(MU_PLUS.yT - 0.4) <400. ";
  // //q1 += "AND MU_MINUS.xT*MU_MINUS.xT +(MU_MINUS.yT - 0.4)*(MU_MINUS.yT - 0.4) <400. ";
...
void extractData(int rs, int index){
   ...
   ifstream ifs;
   oss.str("");
   oss << "list/R008_roadset_" << rs << "_" << index << ".list";
   ...
   while( ifs >> host_tmp >> run >> rs && ifs_R007 >> host_tmp_R007 >> run_R007 >> rs_R007 ){
      ...
      if ( con->SelectDataBase(run.Data()) != 0 )
	{
	  ofs << run << endl;
	  continue;
	}  

      result = con->Query(q1.Data());
...
void extract(int roadset, int index){
